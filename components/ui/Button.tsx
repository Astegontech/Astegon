...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className = '', variant = 'primary', isLoading = false, children, disabled, ...props }, ref) => {
        const baseStyles = 'relative inline-flex items-center justify-center px-6 py-2.5 rounded-full text-sm font-medium transition-colors duration-200 overflow-hidden';
        ...
        return (
            <motion.button
                ...
                className={`${baseStyles} ${variantStyles} ${disabled || isLoading ? 'cursor-not-allowed' : ''} ${className}`}
